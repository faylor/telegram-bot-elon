...
async def fibs_chart(message: types.Message, regexp_command):
    ...
    try:
        ...
        if rsi_df is not None:
            apd.append(mpf.make_addplot(rsi_df, color='#FFFFFF', panel='lower', ylabel='RSI', y_on_right=True, ylim=[0,100]))
...
async def fibs_chart_extended(message: types.Message, regexp_command):
    ...
    try:
        ...
        if rsi_df is not None:
            apd.append(mpf.make_addplot(rsi_df, color='#FFFFFF', panel='lower', y_on_right=True, ylabel='RSI', ylim=[0,100]))
