...
def coin_price(labels):
    try:
        ...
        if response.status_code == 429:
            ...
            logging.error("REPONSE STATUS: " + str(response.status_code))
            logging.error("REPONSE TEXT: " + str(response.text))
            data = response.json()
            data_arr = data["data"]
...
def get_ohcl_trades(coin, period_seconds, period_counts, exchange='binance', pair='usdt'):
    http.headers.clear()
